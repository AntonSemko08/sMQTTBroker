...
void sMQTTBroker::update()
{
	...
	for (clit = clients.begin(); clit != clients.end(); clit++)
	{
		...
		{
			// >>> LWT: публікуємо Will при неочікуваному розриві
			if (c->hasWill && !c->disconnectedGracefully)
			{
				SMQTT_LOGD("LWT publish topic:%s", c->willTopic.c_str());
				publish(c->willTopic, c->willMessage, c->willQos, c->willRetain);
				c->hasWill = false;
			}
			// <<< LWT
...
// ============================================================
// PRIVATE overload — called by sMQTTClient::processMessage()
// when an incoming PUBLISH arrives from a client.
// ============================================================
...
// ============================================================
// PUBLIC overload — used by LWT, rules, timers, $SYS.
// Also fires Public_sMQTTEventType so the sketch can track
// device online/offline state via onEvent().
// ============================================================
void sMQTTBroker::publish(const std::string &topic, const std::string &payload,
                          unsigned char qos, bool retain)
{
	// >>> LWT: єдина точка входу для onEvent()
	{
		sMQTTPublicClientEvent event(nullptr, topic);
	...
	for (sub = subscribes.begin(); sub != subscribes.end(); sub++)
	{
		if ((*sub)->match(topic))
		{
			...
			for (auto cl : subList)
			{
				if(once) { msg.sendTo(cl); once=false; }
				else     { msg.sendTo(cl,false); }
			}
		}
	}
	if (retain)

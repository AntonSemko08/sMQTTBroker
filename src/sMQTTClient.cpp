...
void sMQTTClient::processMessage()
{
	...
	case sMQTTMessage::Type::Connect:
		{
			...
			else
			{
				...
				{
					hasWill = true;
					willRetain = (mqtt_flags & sMQTTWillRetainFlag) != 0;
					willQos = (mqtt_flags & sMQTTWillQoSFlag) >> 3;
					...
					SMQTT_LOGD("LWT topic:%s qos:%d retain:%d", willTopic.c_str(), willQos, willRetain);
				}
				// <<< LWT
	...
	case sMQTTMessage::Type::Disconnect:
		{
			// >>> LWT: graceful disconnect -> Will НЕ публікуємо
